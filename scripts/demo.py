...
<a href="/walkthrough.webm">Watch the walkthrough</a>
                ...
                for route in routes:
                    ...
                    content = re.sub(r'(/api/(?:threads|posts)/\d+|/api/threads)(?=")', r'\1.json', content)
                    content = content.replace('/api/search?q=backup&amp;page=1', '/api/search.json').replace('/api/search?q=backup', '/api/search.json')
